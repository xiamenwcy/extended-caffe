...
void PoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  output_size_=pool_param.output_size();
}
...
void PoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  top[0]->Reshape(bottom[0]->num(), channels_, pooled_height_,
      pooled_width_);
  if (top.size() ==2) {
	  if(output_size_==true)
	  {
		 top[1]->Reshape(1,1,height_,width_);
	  }
	  else
	  {
		 CHECK(this->layer_param_.pooling_param().pool() ==PoolingParameter_PoolMethod_MAX)<< 
         "pooling type must  be max ,for ave type,it cannot output pool index";
		 top[1]->ReshapeLike(*top[0]); 
	  }
  }
  else if(top.size() >2)
  {
	  CHECK(output_size_== true) << 
         "pooling output_size need to be true";
      CHECK(this->layer_param_.pooling_param().pool() ==PoolingParameter_PoolMethod_MAX)<< 
         "pooling type must  be max ,for ave type,it cannot output pool index";
      top[1]->ReshapeLike(*top[0]); 		 
	  top[2]->Reshape(1,1,height_,width_); 
  }
  
  // If max pooling, we will initialize the vector index part.
  if (this->layer_param_.pooling_param().pool() ==
      PoolingParameter_PoolMethod_MAX &&(top.size() ==1 || (top.size()==2&&output_size_==true))) {
